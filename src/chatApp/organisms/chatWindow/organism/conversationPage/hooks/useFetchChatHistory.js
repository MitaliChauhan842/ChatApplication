...
// constants
import CONVERSATION_MAPPING from "../../../../../../data/conversionMappings";
import {
  EMPTY_ARRAY,
  EMPTY_OBJECT,
...
const addNestedChatHistoryNode = (path, parentThread) => (messageInfo) => {
  ...
  const flatThreadChatHistory =
    CONVERSATION_MAPPING[threadConversationId] || EMPTY_ARRAY;
  const updatedPath = [...path, id];
  return {
    ...messageInfo,
    threadMessages: _map(
      flatThreadChatHistory,
      addNestedChatHistoryNode(updatedPath, threadConversationId)
    ),
    path,
    parentThread,
  };
...
const useFetchChatHistory = (selectedUserId, userMetadata) => {
  ...
  useEffect(() => {
    ...
    console.log("flatChatHistory", flatChatHistory);
    const nestedChatHistory = createNestedChatHistoryTree(
      flatChatHistory,
      conversationId
    );
